...
class UNet(Module):
    def __init__(self, channels_in=1, channels_out=1, conv_start_channels=64, depth=4, acti=LeakyReLU(inplace=True)):
        ...
        self.in_block = ConvBlock(channels_in, conv_start_channels, acti)

        self.down_blocks = ModuleList()
        self.up_blocks = ModuleList()

        for i in range(depth):
            down_block = DownBlock(conv_start_channels * pow(2, i),
                                   conv_start_channels * pow(2, i + 1), acti)
            self.down_blocks.add_module(str(i), down_block)

        for i in range(depth, 0, -1):
            up_block = UpBlock(conv_start_channels * pow(2, i),
                               conv_start_channels * pow(2, i - 1), acti)
